...
//riemann integral under curve using Ndiv divisions
double FiniteFunction::integrate(int Ndiv){ //private
  double step = (m_RMax - m_RMin)/(double)Ndiv; // divide range in to Ndiv steps
  double x = m_RMin; // start at first step
  double integral = 0.0; // initialise integral
  for (int i = 0; i < Ndiv; i++){ //
    integral += this->callFunction(x)*step; //return fucntion value @ x and multiply by step size
    x += step;
  }
  return integral; 
}
